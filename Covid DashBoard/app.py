...
tickFont = {'size':14, 'color':"rgb(148, 46, 46)", 'family':"Lucida Console, monospace"}
...
def loadData_GLOB(fileName, columnName): 
    ...
    ## Extract the chinese provinces separately
...
<html>
    <head>
        ...
        <title>COVID-19 Corona Virus Confirmed & Death Cases plots</title>
        {%favicon%}
        {%css%}
    </head>
    <body>
...
app.layout = html.Div(
    ...
    children=[
        html.H1('Corona Virus Confirmed & Death Cases Plots'),
...
def filtered_data(country, state):
    ...
    data['NewDeathsSMA7'] = moving_average(data.NewDeaths, len=7)
    data['NewConfirmedSMA7'] = moving_average(data.NewConfirmed, len=7)
    return data
        ...
        for metric in metrics:
            figure.add_trace(
                go.Scatter(
                    ...
                    name='Rolling avg of Deaths' if metric == 'Deaths' \
                        else 'Rolling avg of Confirmed cases'
                )
            )

def linchart(data, metrics, prefix="", yaxisTitle=""):
    figure = go.Figure(data=[
        go.Bar( 
            name=metric, x=data.date, y=data[prefix + metric],
            marker_line_color='rgb(0,0,0)', marker_line_width=1,
            marker_color={ 'Deaths':'rgb(227, 210, 18)', 'Confirmed':'rgb(18, 116, 227)'}[metric]
        ) for metric in metrics
    ])
...
def update_plots(country, state, metrics, n):
    ...
    linchart_new = linchart(data, metrics, prefix="New", yaxisTitle="New Cases per Day")
    linchart_cum = linchart(data, metrics, prefix="Cum", yaxisTitle="Cumulated Cases")
    return linchart_new, linchart_cum
